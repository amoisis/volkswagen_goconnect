...
- `custom_components/volkswagen_goconnect/` - Main integration code
...
- Integration logs: `custom_components.volkswagen_goconnect: debug` in `config/configuration.yaml`
...
**Validation:** Run `script/lint` before committing (runs type-check + lint + spell)
...
- **Class prefix:** `VolkswagenGoConnect`

**When creating new files:**

- Use the domain `volkswagen_goconnect` for all DOMAIN references
- Prefix all integration-specific classes with `VolkswagenGoConnect`
- Use "Volkswagen GoConnect" as the display title
...
- `tests/` mirrors `custom_components/volkswagen_goconnect/` structure
